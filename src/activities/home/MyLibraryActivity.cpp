...
void MyLibraryActivity::render() const {
  ...
  auto folderName = basepath == "/" ? "Carte SD" : basepath.substr(basepath.rfind('/') + 1).c_str();
  ...
  if (files.empty()) {
    renderer.drawText(UI_10_FONT_ID, metrics.contentSidePadding, contentTop + 20, "Aucun livre trouvé");
  } else {
    GUI.drawList(
  ...
  // Help text
  const auto labels = mappedInput.mapLabels("« Accueil", "Ouvrir", "Haut", "Bas");
