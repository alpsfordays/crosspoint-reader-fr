...
void XtcReaderChapterSelectionActivity::renderScreen() {
  ...
  const int titleX =
      contentX + (contentWidth - renderer.getTextWidth(UI_12_FONT_ID, "Selection de chapitre", EpdFontFamily::BOLD)) / 2;
  renderer.drawText(UI_12_FONT_ID, titleX, 15 + contentY, "Selection de chapitre", true, EpdFontFamily::BOLD);

  const auto& chapters = xtc->getChapters();
  if (chapters.empty()) {
    // Center the empty state within the gutter-safe content region.
    const int emptyX = contentX + (contentWidth - renderer.getTextWidth(UI_10_FONT_ID, "Aucun chapitre")) / 2;
    renderer.drawText(UI_10_FONT_ID, emptyX, 120 + contentY, "Aucun chapitre");
  ...
  for (int i = pageStartIndex; i < static_cast<int>(chapters.size()) && i < pageStartIndex + pageItems; i++) {
    ...
    const char* title = chapter.name.empty() ? "Sans nom" : chapter.name.c_str();
