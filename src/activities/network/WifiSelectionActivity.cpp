...
void WifiSelectionActivity::selectNetwork(const int index) {
  ...
  if (selectedRequiresPassword) {
    ...
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, "Entrez mot de passe :",
        "",     // No initial text
        50,     // Y position
...
void WifiSelectionActivity::renderNetworkList() const {
  ...
  if (networks.empty()) {
    ...
    renderer.drawCenteredText(SMALL_FONT_ID, top + height + 10, "Appuyez Connect. pour rescanner");
  } else {
    ...
    // Show network count
    char countStr[32];
    snprintf(countStr, sizeof(countStr), "%zu réseau·x trouvé·s", networks.size());
  ...
  // Draw help text
  renderer.drawText(SMALL_FONT_ID, 20, pageHeight - 75, "* = Crypté | + = Sauvé");
  const auto labels = mappedInput.mapLabels("« Retour", "Connect.", "", "");
...
void WifiSelectionActivity::renderConnecting() const {
  ...
  if (state == WifiSelectionState::SCANNING) {
    ...
    std::string ssidInfo = "à " + selectedSSID;
...
void WifiSelectionActivity::renderConnected() const {
  ...
  const auto labels = mappedInput.mapLabels("", "Continuer", "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderSavePrompt() const {
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, "Sauvegarder le mot de passe?");
  ...
  // Use centralized button hints
  const auto labels = mappedInput.mapLabels("« Sauter", "Select.", "<", ">");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderConnectionFailed() const {
  ...
  // Use centralized button hints
  const auto labels = mappedInput.mapLabels("« Retour", "Continuer", "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderForgetPrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, "Connexion échouée", true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, "Supprimer le mot de passe?");
  ...
  // Draw "Cancel" button
  if (forgetPromptSelection == 0) {
    renderer.drawText(UI_10_FONT_ID, startX, buttonY, "[Annuler]");
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + 4, buttonY, "Annuler");
  }

  // Draw "Forget network" button
  if (forgetPromptSelection == 1) {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing, buttonY, "[Oublier réseau]");
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, "Oublier réseau");
  }
