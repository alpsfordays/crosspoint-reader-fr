...
void OtaUpdateActivity::render() {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, "Mettre à jour", true, EpdFontFamily::BOLD);

  if (state == CHECKING_FOR_UPDATE) {
    renderer.drawCenteredText(UI_10_FONT_ID, 300, "Vérification...", true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }

  if (state == WAITING_CONFIRMATION) {
    renderer.drawCenteredText(UI_10_FONT_ID, 200, "Nouvelle mise à jour!", true, EpdFontFamily::BOLD);
    ...
    const auto labels = mappedInput.mapLabels("Annuler", "Effectuer", "", "");
  ...
  if (state == NO_UPDATE) {
    renderer.drawCenteredText(UI_10_FONT_ID, 300, "Aucune mise à jour", true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }
  ...
  if (state == FINISHED) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, 350, "Appuyez le bouton d'alimentation");
    renderer.displayBuffer();
    state = SHUTTING_DOWN;
    return;
  }
