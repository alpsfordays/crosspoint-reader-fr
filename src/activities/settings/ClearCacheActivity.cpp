...
void ClearCacheActivity::render() {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, "Vider cache", true, EpdFontFamily::BOLD);

  if (state == WARNING) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 60, "Cette action videra toute la cache", true);
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 30, "Tout progrès sera perdu!", true,
                              EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 + 10, "Chaque livre aura à être réindexé", true);
  ...
  if (state == CLEARING) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2, "Effacement de cache...", true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }

  if (state == SUCCESS) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 20, "Cache vidée", true, EpdFontFamily::BOLD);
    String resultText = String(clearedCount) + " objet·s effacé·s";
    if (failedCount > 0) {
      resultText += ", " + String(failedCount) + " échoué·s";
    }
  ...
  if (state == FAILED) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 20, "Effacement échoué", true, EpdFontFamily::BOLD);
