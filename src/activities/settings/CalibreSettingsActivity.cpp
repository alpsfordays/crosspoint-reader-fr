...
const char* menuNames[MENU_ITEMS] = {"Lien serveur OPDS", "Nom d'utilisateur", "Mot de passe"};
}  // namespace
...
void CalibreSettingsActivity::render() {
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    if (i == 0) {
      ...
      status = (strlen(SETTINGS.opdsUsername) > 0) ? "[Défini]" : "[Indéfini]";
    } else if (i == 2) {
      status = (strlen(SETTINGS.opdsPassword) > 0) ? "[Défini]" : "[Indéfini]";
    }
